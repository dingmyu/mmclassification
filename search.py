...
f = open('search_list.txt').readlines()[int(sys.argv[4]):int(sys.argv[5])]
for line in f:
    params = line.strip()
    command = 'CUDA_VISIBLE_DEVICES={},{} PORT={} bash ./tools/dist_train.sh configs/hrnet.py 2 ' \
              '--net_params {} --work-dir ./output/{}'.format(
                sys.argv[1], sys.argv[2], sys.argv[3], params, params)
    print(command)
